[Freshservice_P1_Teams_Automation_README.md](https://github.com/user-attachments/files/32277145/Freshservice_P1_Teams_Automation_README.md)
...
![Initial P1 incident flow](./images/Flow.png)

### Flow 2 — Check Existing Chat and Create a New Chat When Needed

![P1 chat creation and SharePoint mapping flow](./images/Flow%20Phase%202.png)

### Flow 3 — Post Follow-Up Incident Updates

![P1 follow-up update flow](./images/Flow%20Phase%203.png)

## Technologies Used

| Platform / service | Purpose in the solution |
| --- | --- |
| Freshservice Automator | Detects ticket updates and sends P1 notification emails. |
| Microsoft Power Automate | Orchestrates the workflow, conditions, variables, and error-aware routing. |
| Outlook shared mailbox | Provides the email trigger point between Freshservice and Power Automate. |
| Microsoft Teams | Hosts the dedicated group chat for P1 incident collaboration. |
| SharePoint List | Stores the ticket subject and Teams Chat ID mapping. |
...
1. Read the Freshservice email and extract the ticket subject and incident details.
2. Query the SharePoint incident-chat mapping list for the ticket subject.
3. If a matching item exists, retrieve the saved Teams Chat ID.
4. If no item exists:
...
5. Post subsequent incident updates to the existing chat.
